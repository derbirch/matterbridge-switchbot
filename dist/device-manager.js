export class DeviceManager {
    ...
    getDevicesByType(type) {
        return Array.from(this.devices.values()).filter(device => device.type.toLowerCase().includes(type.toLowerCase()));
    }
    /**
    ...
    async updateAllDeviceStatuses() {
        const updatePromises = Array.from(this.devices.keys()).map(deviceId => this.updateDeviceStatus(deviceId));
        await Promise.allSettled(updatePromises);
    }
    /**
     * Send command to device
     */
    async sendDeviceCommand(deviceId, command, parameter) {
        const device = this.devices.get(deviceId);
        if (!device) {
            this.log.warn(`Device ${deviceId} not found`);
            return false;
     ...
     */
    createManagedDevice(device, isInfrared) {
     ...
     */
    determineCapabilities(deviceType, isInfrared) {
     ...
     */
    isCommandSupported(deviceId, command) {
