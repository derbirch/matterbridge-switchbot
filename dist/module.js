import { MatterbridgeDynamicPlatform, MatterbridgeEndpoint, onOffOutlet, onOffSwitch, colorTemperatureLight } from 'matterbridge';
import { SwitchBotApi } from './switchbot-api.js';
/**
...
export class SwitchBotPlatform extends MatterbridgeDynamicPlatform {
    ...
    constructor(matterbridge, log, config) {
        ...
        if (config.token && config.secret) {
            const switchBotConfig = {
                ...
                baseUrl: config.baseUrl || 'https://api.switch-bot.com',
            };
    ...
    async registerSwitchBotDevice(device) {
    ...
    async registerInfraredDevice(device) {
    ...
    createSwitchDevice(device) {
        const endpoint = new MatterbridgeEndpoint(onOffSwitch, { uniqueStorageKey: device.deviceId })
            .createDefaultBridgedDeviceBasicInformationClusterServer(device.deviceName, device.deviceId, this.matterbridge.aggregatorVendorId, 'SwitchBot', device.deviceType, 1, '1.0.0')
    ...
    createInfraredSwitchDevice(device) {
        const endpoint = new MatterbridgeEndpoint(onOffSwitch, { uniqueStorageKey: device.deviceId })
            .createDefaultBridgedDeviceBasicInformationClusterServer(device.deviceName, device.deviceId, this.matterbridge.aggregatorVendorId, 'SwitchBot IR', device.remoteType, 1, '1.0.0')
            .createDefaultPowerSourceWiredClusterServer()
            .addRequiredClusterServers()
            .addCommandHandler('on', async () => {
            await this.sendCommand(device.deviceId, { command: 'turnOn' });
    ...
    updateMatterDeviceState(device, status) {
    ...
    startPolling() {
        const pollIntervalMs = (this.config.pollInterval || 30) * 1000;
